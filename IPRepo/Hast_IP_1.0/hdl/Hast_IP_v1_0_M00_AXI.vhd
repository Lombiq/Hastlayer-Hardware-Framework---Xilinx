...
entity Hast_IP_v1_0_M00_AXI is
	generic (
		...
		C_M_TARGET_SLAVE_BASE_ADDR : std_logic_vector := x"48000000";  -- Base address of targeted slave
...
end Hast_IP_v1_0_M00_AXI;

architecture implementation of Hast_IP_v1_0_M00_AXI is
	...
	constant  C_TRANSACTIONS_NUM : integer := 0; --clogb2(C_M_AXI_BURST_LEN-1);
	-- Burst length for transactions, in C_M_AXI_DATA_WIDTHs.
	-- Non-2^n lengths will eventually cause bursts across 4K address boundaries.
	constant  C_MASTER_LENGTH  : integer := 12;
	-- total number of burst transfers is master length divided by burst length and burst size
	constant  C_NO_BURSTS_REQ  : integer := 0; --(C_MASTER_LENGTH-clogb2((C_M_AXI_BURST_LEN*C_M_AXI_DATA_WIDTH/8)-1)); --@ Hastlayer: tried to set this to 1 but it just overwrites memory
	...
	signal sig_num_reads            : integer;

begin
	-- I/O Connections assignments

	M_AXI_AWID       <= (others => '0'); --I/O Connections. Write Address (AW)
	M_AXI_AWADDR     <= std_logic_vector(unsigned(C_M_TARGET_SLAVE_BASE_ADDR) + unsigned(axi_awaddr)); --The AXI address is a concatenation of the target base address + active offset range
	M_AXI_AWLEN      <= std_logic_vector(to_unsigned(C_M_AXI_BURST_LEN - 1, 8)); --Burst LENgth is number of transaction beats, minus 1
	M_AXI_AWSIZE     <= std_logic_vector(to_unsigned(clogb2((C_M_AXI_DATA_WIDTH/8)-1), 3)); --Size should be C_M_AXI_DATA_WIDTH, in 2^SIZE bytes, otherwise narrow bursts are used
	M_AXI_AWBURST    <= "01"; --INCR burst type is usually used, except for keyhole bursts
	M_AXI_AWLOCK     <= '0';
	M_AXI_AWCACHE    <= "0010";  --Update value to 4'b0011 if coherent accesses to be used via the Zynq ACP port. Not Allocated, Modifiable, not Bufferable. Not Bufferable since this example is meant to test memory, not intermediate cache.
	M_AXI_AWPROT     <= "000";
	M_AXI_AWQOS      <= x"0";
	M_AXI_AWUSER     <= (others => '1');
	M_AXI_AWVALID    <= axi_awvalid;
	M_AXI_WDATA      <= Hast_IP_Write_Data_in; --Write Data(W)
	M_AXI_WSTRB      <= (others => '1'); --All bursts are complete and aligned in this example
	M_AXI_WLAST      <= axi_wlast;
	M_AXI_WUSER      <= (others => '0');
	M_AXI_WVALID     <= axi_wvalid;
	M_AXI_BREADY     <= axi_bready; --Write Response (B)
	M_AXI_ARID       <= (others => '0'); --Read Address (AR)
	M_AXI_ARADDR     <= std_logic_vector(unsigned(C_M_TARGET_SLAVE_BASE_ADDR) + unsigned(axi_araddr));
	M_AXI_ARLEN      <= std_logic_vector( to_unsigned(C_M_AXI_BURST_LEN - 1, 8));  --Burst LENgth is number of transaction beats, minus 1
	M_AXI_ARSIZE     <= std_logic_vector( to_unsigned( clogb2((C_M_AXI_DATA_WIDTH/8)-1),3 )); --Size should be C_M_AXI_DATA_WIDTH, in 2^n bytes, otherwise narrow bursts are used
	M_AXI_ARBURST    <= "01"; --INCR burst type is usually used, except for keyhole bursts
	M_AXI_ARLOCK     <= '0';
	M_AXI_ARCACHE    <= "0011"; --Update value to 4'b0011 if coherent accesses to be used via the Zynq ACP port. Not Allocated, Modifiable, not Bufferable. Not Bufferable since this example is meant to test memory, not intermediate cache.
	M_AXI_ARPROT     <= "000";
	M_AXI_ARQOS      <= x"0";
	M_AXI_ARUSER     <= (others => '1');
	M_AXI_ARVALID    <= axi_arvalid;
	M_AXI_RREADY     <= axi_rready; --Read and Read Response (R)
	TXN_DONE         <= compare_done; --Example design I/O
	burst_size_bytes <= std_logic_vector( to_unsigned((C_M_AXI_BURST_LEN * (C_M_AXI_DATA_WIDTH/8)),C_TRANSACTIONS_NUM+3)); --Burst size in bytes
	init_txn_pulse   <= INIT_AXI_TXN; --( not init_txn_ff2)  and  init_txn_ff;

	Hast_IP_Num_Of_Reads_out <= std_logic_vector(to_unsigned(sig_num_reads,32));
	...
	begin
		if (rising_edge (M_AXI_ACLK)) then
			if (M_AXI_ARESETN = '0' or init_txn_pulse = '1') then
				axi_awaddr <= Hast_IP_Address_in; --(others => '0');
			else
	...
	begin
		if (rising_edge (M_AXI_ACLK)) then
			if (M_AXI_ARESETN = '0' or init_txn_pulse = '1' ) then
				axi_araddr <= Hast_IP_Address_in; --(others => '0');
			else
			...
			-- when M_AXI_RVALID is asserted by slave
				...
				if (M_AXI_RVALID = '1') then
					if (M_AXI_RLAST = '1' and axi_rready = '1') then
						...
						axi_rready <= '1';
						Hast_IP_Read_Data_out <= M_AXI_RDATA;
					end if;
				end if;
			end if;
		end if;
		...
		if (rising_edge (M_AXI_ACLK)) then
			if (M_AXI_ARESETN = '0') then
				...
				case (mst_exec_state) is
					...
					when IDLE =>
						Hast_IP_SM_State_out <= "00";
						-- This state is responsible to initiate
						-- AXI transaction when init_txn_pulse is asserted
						if ( init_txn_pulse = '1') then
							if (Hast_IP_Read_Write_Mode_in = '1') then
	...
	begin
		if (rising_edge (M_AXI_ACLK)) then
			if (M_AXI_ARESETN = '0' or init_txn_pulse = '1') then
				...
				--if (M_AXI_BVALID = '1' and (write_burst_counter(0) = '1') and axi_bready = '1') then
	...
	-- Add user logic here
	Hast_IP_Writes_Done_out <= writes_done and not INIT_AXI_TXN;
	Hast_IP_Reads_Done_out <= reads_done and not INIT_AXI_TXN;
	-- User logic ends
